...
namespace SalesSystem.Areas.Users.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            if(_dataUser1 != null)
            {
                Input = new InputModel
                {
                    ...
                    PhoneNumber = _dataUser1.PhoneNumber,
        ...
        //metodo por el que registras la informacion del usuario por Registro.cshtml con el post

        public async Task<IActionResult> OnPost(String dataUser)
        {
            if (dataUser == null)
            {
                // se crea a condicion,ejecuta la tarea de registrar los usuarios, si me retorna verdadero
                //el usuario se HA registrado y esta en las tablas de LA db, ademas nos redirecciona a el siguiente link
                if (await SaveAsync())
                {
            ...
            {
                //obtiene los datos de un usuario y los muestra en el registro para actualizar datos
        ...
        // este metodo lo que hace es salvar los datos en las tablas

        public async Task<bool> SaveAsync()
        {
            _dataInput = Input;
            var valor = false;
            //esta condicional lo que hace es con el ModelState es capturar la informacion enviada x el post
            //si la infomacion que metemos es correcta (true) se guarda sino no me deja seguir
                    ...
                    await strategy.ExecuteAsync(async () =>
                    {
                        using (var transaction = _context.Database.BeginTransaction())
                        {
                            try
                            {
                                ...
                                if (result.Succeeded)
                                {
                                    //pone el role al usuari que se le mete a la tabla
                                    await _userManager.AddToRoleAsync(user, Input.Role);
                                    //meto en una variable la comprobacion de de los datos en la tabla 
                                    //del email,si existe o no, comprobandolo desde el ultimo
        ...
        private async Task<bool> UpdateAsybc()
        {
            var valor = false;
            byte[] imageByte = null;
            var strategy = _context.Database.CreateExecutionStrategy();
